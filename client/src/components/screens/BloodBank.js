...
function BloodBank() {
    ...
    const addBloodGroup = (e) => {
        ...
        fetch('http://localhost:5000/api/blood/add', fetchData)
        .then(res => res.json())
        .then(data2 => {
            setLoading(false);
            if(data2.error) {
    ...
    const bookBlood = (bloodId) => {
        ...
        fetch('http://localhost:5000/api/booking/new', {
            method: "post",
            headers: {
                "Content-Type": "application/json",
                "Authorization":"Bearer "+JSON.parse(localStorage.getItem("jwt"))
    ...
    const deleteBlood = (bloodId) => {
        ...
        fetch(`http://localhost:5000/api/blood/delete/${bloodId}`, fetchData)
        .then(res => res.json())
        .then(data2 => {
            if(data2.error) {
                toast.error(data2.error)
